...
/** END Custom change: remember unresolved classes */
  ...
  public boolean isAssignableFrom(IClass c1, IClass c2);
     ...
     * added by imcczy
     */
    public Collection<IClass> getIClass();
    //added by imcczy
    public Map<TypeReference, ClassHierarchy.Node> getMap();
    //added by imcczy
    public static IClass lookupClassRecursive(TypeReference a, Map<TypeReference, ClassHierarchy.Node> map, IClassLoader iClassLoader) {
/** END Custom change: remember unresolved classes */
        //ClassLoaderReference loader = a.getClassLoader();

        ClassLoaderReference parent = a.getClassLoader().getParent();
        // first delegate lookup to the parent loader.
        if (parent != null) {
            TypeReference p = TypeReference.findOrCreate(parent, a.getName());
            //TypeReference test = TypeReference.find(ClassLoaderReference.Primordial,"Ljava/lang/String");
            IClass c = lookupClassRecursive(p, map, iClassLoader);
            if (c != null) {
                return c;
            }
        }
        ...
        if (a.isArrayType()) {
            ...
            if (elt.isPrimitiveType()) {
                ...
                return iClassLoader.lookupClass(a.getName());
            } else {
                IClass c = lookupClassRecursive(elt, map, iClassLoader);
                if (c == null) {
            ...
            ClassHierarchy.Node n = map.get(a);
            if (n != null) {
                return n.getJavaClass();
            } else {
                return null;
            }
        }
